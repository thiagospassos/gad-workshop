<template>
  <div class="invalid-feedback">
    <div v-if="containsKey('isUnique') && !model.isUnique">
      {{ field }} already exists.
    </div>
    <div v-if="containsKey('required') && !model.required">
      {{ field }} is a required field.
    </div>
    <div v-if="containsKey('decimal') && !model.decimal">
      {{ field }} is of type decimal.
    </div>
    <div v-if="containsKey('numeric') && !model.numeric">
      {{ field }} is of type numeric.
    </div>
    <div v-if="containsKey('minValue') && !model.minValue">
      {{ field }} must be equal or greater than
      {{ model.$params.minValue.min }}.
    </div>
    <div v-if="containsKey('minLength') && !model.minLength">
      {{ field }} must be at least {{ model.$params.minLength.min }} characters.
    </div>
    ...
    <template v-for="validator in customValidatorKeys">
      <div :key="validator" v-if="!model[validator]">
        {{ model.$params[validator].message }}
      </div>
    </template>
  </div>
</template>
